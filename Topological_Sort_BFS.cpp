https://practice.geeksforgeeks.org/problems/topological-sort/1/?track=DSASP-Graph&batchId=154#
// 8. Topological sort 
// Medium Accuracy: 40.0% Submissions: 76590 Points: 4
// Given a Directed Acyclic Graph (DAG) with V vertices and E edges, Find any Topological Sorting of that Graph.


// Your Task:
// You don't need to read input or print anything. Your task is to complete the function topoSort()  which takes the integer V denoting the number of vertices and adjacency list as input parameters and returns an array consisting of a the vertices in Topological order. As there are multiple Topological orders possible, you may return any of them. If your returned topo sort is correct then console output will be 1 else 0.
// Expected Time Complexity: O(V + E).
// Expected Auxiliary Space: O(V).


// Constraints:
// 2 ≤ V ≤ 104
// 1 ≤ E ≤ (N*(N-1))/2

class Solution{
	public:
	//Function to return list containing vertices in Topological order. 
	vector<int> topoSort(int V, vector<int> adj[]) 
	{
	    vector<int> res;
        vector<int> inorder(V, 0); // V length intiallized with zeros
        queue<int> q; 
        
        for(int i = 0; i < V; i++)
            for(int u: adj[i])  inorder[u]++;
            
        for(int i = 0; i < V; i++)
            if(inorder[i] == 0)  q.push(i);
        
        while(!q.empty()){
            int u = q.front(); res.push_back(u);
            for(int v: adj[u]){
                inorder[v]--;
                if(inorder[v] == 0) q.push(v);
            }
            q.pop();
        }
        return res;
	}
};